...
def read_data(path_to_csvs: str, load = True) -> pd.DataFrame:
    ...
    if os.path.exists(super_file) and load:
         return pd.read_csv(super_file, low_memory=False)
    ...
    # Save unique product names (optional)
    with open(os.path.join(path_to_csvs, "unique_drugs.txt"), "w") as fh:
        for v in pd.Series(df.get("Product Name", pd.Series(dtype=object)).unique()).dropna().sort_values():
            fh.write(f"{v}\n")
...
def plot_correlation_p_values(df: pd.DataFrame):
    """
    Calculates, prints, and plots p-values and their sample sizes (N) for the
    correlation between prescription numbers and tax data for each drug.

    Args:
        df (pd.DataFrame): Input DataFrame with columns 'Product Name',
                           'Number of Prescriptions', 'dollar_tax', 'percent_tax'.
    """
    # --- 1. Calculate p-values and sample sizes (N) ---
    print("Calculating p-values and sample sizes (N)...")

    # Helper function to safely calculate p-value and N
    def get_p_value_and_n(x, y):
        pairs = pd.DataFrame({"x": x, "y": y}).dropna()   # <-- actually drop NaNs
        n = len(pairs)
        if n < 3 or pairs["x"].nunique() < 2 or pairs["y"].nunique() < 2:
            return (np.nan, n)
        return pearsonr(pairs["x"], pairs["y"])[1], n

    # A single worker function for apply to avoid redundant calculations
    def calculate_stats(g):
        dollar_p, dollar_n = get_p_value_and_n(g['dollar_tax'], g['Number of Prescriptions'])
        percent_p, percent_n = get_p_value_and_n(g['percent_tax'], g['Number of Prescriptions'])
        return pd.Series({
            'dollar_tax_p_value': dollar_p,
            'N_dollar_tax': dollar_n,
            'percent_tax_p_value': percent_p,
            'N_percent_tax': percent_n
        })

    # Apply the calculation to each drug group
    p_value_df = df.groupby('Product Name').apply(calculate_stats).reset_index().dropna(
        subset=['dollar_tax_p_value', 'percent_tax_p_value']
    )
    
    # Convert N columns to integers for cleaner display
    if not p_value_df.empty:
        p_value_df = p_value_df.astype({'N_dollar_tax': int, 'N_percent_tax': int})

    if p_value_df.empty:
        print("Could not calculate any p-values. Check if data has enough variance.")
        return

    print("\nCalculated P-Values and Sample Sizes (N):")
    print(p_value_df)

    # --- 2. Plot the p-values (the plotting logic remains the same) ---
    print("\nGenerating plot...")
    p_value_df = p_value_df.sort_values('dollar_tax_p_value', ascending=False)

    drug_names = p_value_df['Product Name']
    y_pos = np.arange(len(drug_names))
    bar_height = 0.35
    ...
    ax.barh(y_pos - bar_height/2,
            p_value_df['dollar_tax_p_value'],
            bar_height,
            label='Dollar Tax vs. Prescriptions P-Value',
            color='deepskyblue')

    ax.barh(y_pos + bar_height/2,
            p_value_df['percent_tax_p_value'],
            bar_height,
    ...
    ax.set_title('Statistical Significance (P-Value) of Tax Correlation to Prescriptions')
    
    ax.axvline(0.05, color='red', linestyle='--', linewidth=1.2, label='Significance Threshold (p=0.05)')
    ax.legend()

    ax.invert_yaxis()
    plt.tight_layout()
    
    # Ensure PLOTS_DIR is defined before saving
